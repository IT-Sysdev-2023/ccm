...
<template>
    ...
    <TreasuryLayout>
        ...
        <a-modal v-model:open="openModal" title="Basic Modal" width="80%" wrap-class-name="full-modal" :footer="null">
            <a-row :gutter="[16, 16]">
                <a-col :span="8">
                    <a-breadcrumb>
                        <a-breadcrumb-item href="">
...
export default {
    ...
    data() {
        return {
            ...
            dataClone: JSON.parse(localStorage.getItem('dataCloneFromDB')),
            checkedLocal: [],
        }
    },
    ...
    watch: {
        data: {
            handler(newValue, oldValue) {
                const r = localStorage.getItem('switchControl');
                const c = JSON.parse(r).map(function (data) { return data.checks_id });
                this.checkedLocal = c;
            },
        }
    },
    // mounted() {
    //     localStorage.setItem('dataCloneFromDB', JSON.stringify(this.data));
    // },
    methods: {
        ...
        getCheckedState(record) {
            const storedValue = localStorage.getItem(`isChecked_${record.id}`);
            // If there's no stored value, default to false
        ...
        merginCheckSwitch(record) {
            ...
            localStorage.setItem('dataCloneFromDB', JSON.stringify(this.dataClone));
            // localStorage.setItem('switchControl', JSON.stringify(this.checkedRecords));

            // console.log(localStorage.getItem('switchControl'));
